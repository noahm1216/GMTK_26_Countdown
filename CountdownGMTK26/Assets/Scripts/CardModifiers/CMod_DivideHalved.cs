...
public class CMod_DivideHalved : CardModifierBase
{
        public override float OnCardSelected(CardData _cardData, float _timeCur = -1)
        {
            if (timesUsableInGame < timesApplied || timesUsableInGame == -1) // checks if card is used too many times
            {
                ...
                {
                    if (_cardData.effect == CardEffectType.DivideTime) // if condition is met
                    { timesApplied++; return _cardData.value * 0.5f; } // return value
                }
            }
